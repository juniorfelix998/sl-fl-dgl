...
class Net(nn.Module):
    def __init__(self, depth=110, num_classes=200, num_splits=3):
        ...
        aux_channel_map = {0: 16, 1: 32, 2: 64}
            ...
            if splits_id < num_splits - 1:
                self.auxillary_nets.append(
                    auxillary_classifier2(
                        input_features=aux_channel_map.get(splits_id, 64),
                        in_size=8,
...
def get_tiny_imagenet_loaders(num_clients, batch_size, data_dir=None, alpha=None):
    ...
    # ============================================================
    if alpha is None:
        # --- Stratified IID split (now matches CIFAR pattern) ---
